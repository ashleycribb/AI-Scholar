...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handlePaperReceived = (paper: any) => {
      updatePaperState(paper.id, paper);
    };
          ...
          <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
              <main className={hasSearched ? "lg:col-span-3" : "lg:col-span-5"}>
                {!hasSearched ? (
                  <InitialSearchScreen query={query} onQueryChange={setQuery} onSearch={handleSearch} isLoading={isLoading} summaryLength={summaryLength} onLengthChange={(l) => useStore.setState({ summaryLength: l })} summaryStyle={summaryStyle} onStyleChange={(s) => useStore.setState({ summaryStyle: s })} model={model} onModelChange={setModel} availableModels={AVAILABLE_MODELS} logAnalyticsEvent={() => {}} onOpenDbFinder={openDbFinder} searchSources={searchSources}>
                    <ExtensionPromo />
                  </InitialSearchScreen>
                ) : (
                  <div className="space-y-6">
                    <SearchForm query={query} onQueryChange={setQuery} onSearch={handleSearch} isLoading={isLoading} summaryLength={summaryLength} onLengthChange={(l) => useStore.setState({ summaryLength: l })} summaryStyle={summaryStyle} onStyleChange={(s) => useStore.setState({ summaryStyle: s })} model={model} onModelChange={setModel} availableModels={AVAILABLE_MODELS} logAnalyticsEvent={() => {}} />
