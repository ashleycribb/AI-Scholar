...
function handleFindPdf(e: Event) {
    ...
    chrome.runtime.sendMessage({ action: 'findOpenAccessForPaper', paper: currentPaper }, (response: any) => {
...
function handleGetSuggestions(e: Event) {
    ...
    chrome.runtime.sendMessage({ action: 'getSuggestionsForPaper', paper: currentPaper }, (response: any) => {
...
function renderCoPilotContent(summary: string, analysis: PaperAnalysis) {
    const content = document.getElementById('are-copilot-content') as HTMLElement;
    if (!content) return;
...
export function showCoPilotPanelForPaper(paper: ResearchPaper) {
    currentPaper = paper;
    if (!document.getElementById('are-copilot-panel')) createCoPilotPanel();

    coPilotPanel = document.getElementById('are-copilot-panel');
    ...
    chrome.runtime.sendMessage({ action: 'getAiAnalysisForPaper', paper: currentPaper }, (response: any) => {
        if (response?.success) {
