...
export const PaperCard: React.FC<PaperCardProps> = ({ paper, isFavorite, onToggleFavorite, onFindConnectedPapers, isFindingConnected, onAnalyzePaper, isAnalyzingPaper }) => {
    ...
    <article className="bg-white p-6 rounded-lg shadow-md border border-gray-200 transition-shadow hover:shadow-lg relative">
      ...
      <div className="mt-4 pt-4 border-t border-gray-100 flex justify-end items-center gap-3">
        ...
        <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 text-sm font-medium rounded-full hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors disabled:bg-gray-200 disabled:text-gray-500 disabled:cursor-wait"
            aria-label={`Analyze structure of ${paper.title}`}
        >
             {isAnalyzingPaper ? (
                <>
        ...
        {paper.pdfURL && (
            <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 bg-red-50 text-red-700 text-sm font-medium rounded-full hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors"
